...
name: intake
description: >-
  Resolve a task description (ticket ID, URL, or free-form prompt) into structured task_type, size, summary, acceptance criteria, validated success criteria, and cross-team impact. Use first when starting any new development task, or as the first step of the feature orchestrator.
---
...
1. **Resolve the source.**
   - Ticket-key shape (`^[A-Z]+-\d+$`): if an Atlassian MCP is available, fetch the issue. Otherwise treat as a key the user must paste content for.
   - URL: detect Jira / Linear / GitHub Issues / GitHub PR; use the appropriate MCP or `gh issue view` / `gh pr view`.
   - Free-form: the user _is_ the source.

2. **Extract acceptance criteria.** Pull explicit AC if present. Otherwise infer 3–5 testable criteria from the description.

3. **Classify task type.** Pick one of: `feature`, `bug`, `release`, `review`. Default to `feature` if ambiguous.
...
```
Type:    <feature|bug|release|review>
...
- **Ticket fetch failed:** fall back to free-form; flag that AC are inferred.
