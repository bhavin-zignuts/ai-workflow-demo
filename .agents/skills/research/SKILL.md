...
name: research
description: >-
  Codebase exploration and hypothesis validation. Auto-detect project conventions, trace lifecycles (sync + async), produce evidence-backed findings before planning. Use as step 2 of feature skill, between intake and plan.
---
...
- Check `CLAUDE.md`, `AGENTS.md`, and any other guidelines for constraints. Heed deprecation notices.
...
4. Cross-repo: if the task touches API contracts, check the other side.

### Step 3 — Auto-detect conventions

Use codebase search to detect: language, framework, styling approach, state management, package manager. Record as `## Project Conventions`. Do not hardcode assumptions — read the repo.
...
### Project conventions
<language, framework, styling, package manager>
...
- **Convention auto-detect picks up legacy files:** check `package.json` / config files for the project's _target_ conventions, not just neighboring code.
