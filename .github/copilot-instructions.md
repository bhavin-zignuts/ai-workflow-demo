...
* **Design Guidelines**: Refer to the `landing-design-system` skill ([SKILL.md](.agents/skills/landing-design-system/SKILL.md)) for landing page color palettes, typography tracking values, and tokens. Refer to the `frontend-design` skill ([SKILL.md](.agents/skills/frontend-design/SKILL.md)) for intentional, anti-cliché UI/UX decisions, copywriting, and visual hierarchy. Refer to the `react-development` skill ([SKILL.md](.agents/skills/react-development/SKILL.md)) for state architecture, compound components, and performance optimization.
* **Accessibility (WCAG 2.2 AA)**: Refer to the `accessibility` skill ([SKILL.md](.agents/skills/accessibility/SKILL.md)) for semantic HTML (no `div` buttons), visible focus rings (`focus-visible:`), proper `aria-` labels for icon buttons, and modal focus management.
